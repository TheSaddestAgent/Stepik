...
class MyTreeClf:
    ...
    def __str__(self):
        ...
        for k, v in self.__dict__.items():
            params.append(f'{k}={v}')
        return f"MyTreeClf class: {', '.join(params)}"
    ...
    def information_gain(self, left_y, right_y, parent_impurity):
        ...
        ig = parent_impurity - (p_left * self.impurity(left_y) + p_right * self.impurity(right_y))
    ...
    def get_best_split(self, X, y):
        ...
        for col in X.columns:
            ...
            for i in range(1, len(unique_values)):
                ...
                ig = self.information_gain(left_y, right_y, self.impurity(y))
    ...
    def fit(self, X, y):
        ...
        def build(root, X_root, y_root, side='root', depth=0):
            ...
            if y_1 is None or y_2 is None or len(y_1) == 0 or len(y_2) == 0:
                ...
                self.fi[best_col] += len(y_root) / len(y) * (self.impurity(y_root) - len(y_1) / len(y_root) * self.impurity(y_1) - len(y_2) / len(y_root) * self.impurity(y_2))
            return root
